...
//  private static final Logger logger = Logger.getLogger(YCSBLevelDBClient.class.getName());
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    try {
      ...
      result =  deserialize(response.getOutput());
  ...
  public Status scan(String table, String startkey, int recordcount, Set<String> fields,
    ...
    ScanM request = ScanM.newBuilder().setTable(table).setStartkey(startkey).setRecordcount(recordcount).build();
    ReadResult response;
    try {
      ...
      int d = output.length()/recordcount;
      for (int i=0; i<recordcount; i++) {
        String str = output.substring(i*d, (i+1)*d);
        HashMap<String, ByteIterator> c = new HashMap<String, ByteIterator>(deserialize(str));
        result.add(c);
      }
    } catch (StatusRuntimeException e) {
      return Status.ERROR;
